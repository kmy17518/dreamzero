# LIBERO Soft Eval — **Explicit Conditioning** (grounded feedback `C`, replan 24), from an empty box

Self-contained recipe to evaluate the **explicit-conditioning (future-frame-shift)** DreamZero‑LIBERO
checkpoint on a **single 80 GB GPU** with **grounded cross‑query feedback (`context_mode=C`)** and the
aligned block cadence (**`replan_steps=24`**), producing both the usual **binary success rate** *and* the
**soft / progress (partial‑stage) scores** added in `eval_utils/run_libero_eval.py`. Runs all **4 eval
task suites** (`libero_spatial`, `libero_object`, `libero_goal`, `libero_10`) with **5 trials/task** and
writes **detailed per‑task metrics**.

> This is the **explicit‑conditioning** counterpart of the baseline `docs/LIBERO_SOFT_EVAL` recipe. The
> only deltas are: check out `libero_explicit_conditioning`, serve a `future_frame_shift` checkpoint with
> **`--context_mode C`**, and run the client with **`--replan-steps 24`**. The progress‑score code, the
> conda envs, the Wan‑backbone download, and the `metrics.json` schema are identical to the baseline.
> Read **`docs/LIBERO_EXPLICIT_CONDITIONING.md` §1.5 / §4** for *why* `C` + `replan 24` (the grounded
> feedback derivation and cadence); this file is the **from‑empty‑box run recipe**.

Assumes a fresh instance: **no conda env, no data, no checkpoints.** Eval needs **(1) the fine‑tuned
`future_frame_shift` checkpoint**, **(2) the umt5 tokenizer**, and **(3) the Wan backbone component
weights** (DiT + T5 text encoder + VAE + CLIP image encoder). It does **not** need the training dataset.

> ⚠️ **The Wan backbone IS required at eval time.** The checkpoint's `config.json` carries **absolute**
> `*_pretrained_path` entries that point at `…/checkpoints/Wan2.2-TI2V-5B` and
> `…/checkpoints/Wan2.1-I2V-14B-480P`. At construction the action head **loads those backbone weights
> first**, then overlays the fine‑tuned safetensors. The VAE / T5 / CLIP loaders fall back to an HF
> download if their local file is missing, but the **diffusion DiT loader has no fallback** — if
> `./checkpoints/Wan2.2-TI2V-5B/` exists (it will, after you drop the tokenizer there) it *requires*
> `diffusion_pytorch_model.safetensors.index.json` + shards to be present locally, or it raises
> `No safetensors file found …`. So §2 downloads the backbone explicitly. Because the repo is cloned at
> `$HOME/dreamzero` and the config paths are `…/dreamzero/checkpoints/…`, keep `$DZ` at `$HOME/dreamzero`
> (or edit `checkpoint-*/config.json` to match your path).
...
> What "explicit conditioning / `context_mode=C`" adds: the server forces a fresh real‑anchored sequence
> every query, feeds back **only the single previously‑generated frame** `Ĝ_{t-1}` as clean context, and
> generates the next video block + aligned action chunk (see `LIBERO_EXPLICIT_CONDITIONING.md` §1.5). It
> **requires** a `future_frame_shift` checkpoint (auto‑detected) and must be paired with
> **`--replan-steps 24`** (`= num_action_per_block`, so 1 query = 1 block ≈ the next real observation).
...
- **Clone the repo and check out the `libero_explicit_conditioning` branch** (where the future‑shift +
  `context_mode=C` *and* the soft‑eval code live). Pick any location; this guide uses `$HOME/dreamzero`:
  ```bash
  export DZ=$HOME/dreamzero
  git clone git@github.com:kmy17518/dreamzero.git "$DZ"   # or https://github.com/kmy17518/dreamzero.git
  cd "$DZ" && git checkout libero_explicit_conditioning
  ```
- **`$DZ/.env`** (git‑ignored) with a Hugging Face token that can read the checkpoint mirror you use in
  §2 (**read scope is enough** — eval never uploads). If you trained locally and point §2 at your own
  `OUTPUT_DIR`, no read scope is needed:
  ```
  HF_TOKEN=hf_...
  ```
- **GPU arch** (informational only — §3 defaults to the **eager** server on every arch):
  ```bash
...
client, py3.10). Identical to the baseline soft‑eval. If you have no conda:
```bash
...
Same as the baseline, but the **checkpoint is a `future_frame_shift` model** — point `MIRROR_REPO` at
your explicit‑conditioning Hub mirror (the watcher's `UPLOAD_REPO` from
`LIBERO_EXPLICIT_CONDITIONING.md` §4.1), **or skip (a)–(b) entirely and use a locally‑trained
checkpoint** at `checkpoints/dreamzero_libero_wan22_future_shift/checkpoint-N`.

```bash
conda activate dreamzero && cd "$DZ" && set -a; . ./.env; set +a
export HF_HUB_ENABLE_HF_TRANSFER=1
mkdir -p checkpoints
export MIRROR_REPO=<your-hf-user>/dreamzero-libero-future-shift-best   # your explicit-conditioning mirror
...
steps = {int(s.rfilename.split("/")[0].split("-")[1])
         for s in a.repo_info(os.environ["MIRROR_REPO"], repo_type="model").siblings
...
hf download "$MIRROR_REPO" \
    --include "checkpoint-$CKPT_STEP/*" \
    --exclude "checkpoint-$CKPT_STEP/global_step*/*" \
    --local-dir ./checkpoints/dreamzero_libero_future_shift_eval
...
>
> **`future_frame_shift` is auto‑detected.** A future‑shift checkpoint records `future_frame_shift: true`
> in its `config.json`, so the server auto‑primes (`context_mode=C` *requires* this). If you ever serve a
> non‑shift checkpoint and still want `C`, force it with `--future_frame_shift True`.

---

## 3. Run the standalone soft eval (single GPU, grounded `C` + replan 24)
...
### Terminal A — policy server (GPU 0), `context_mode=C`
```bash
conda activate dreamzero && cd "$DZ" && set -a; . ./.env; set +a
...
export CKPT_STEP=${CKPT_STEP:?set this to the step you downloaded, e.g. 32000}
export CKPT=$PWD/checkpoints/dreamzero_libero_future_shift_eval/checkpoint-$CKPT_STEP
# (locally-trained instead? export CKPT=$PWD/checkpoints/dreamzero_libero_wan22_future_shift/checkpoint-$CKPT_STEP)

# EAGER server (default, every arch). Add --context_mode C for grounded feedback, and (optional)
# --save_video_pred to dump the model's imagined video per episode (inspect what C feeds back).
...
```
Wait for `server listening on 0.0.0.0:8000` before starting Terminal B. On startup it logs
`future_frame_shift ON: …` and `context_mode=C (grounded explicit conditioning): …`; per episode it then
logs exactly **one** `[future_frame_shift] episode start: returning no-op …` and **no** cache‑reset
regenerations (each later query returns the aligned chunk directly). (First start also fetches any
missing VAE/T5/CLIP into the HF cache; with §2 done it just loads the local files.)

### Terminal B — sim client: all 4 suites × 5 trials/task, `replan 24` + progress scores
```bash
conda activate dreamzero_libero && cd "$DZ"
export OUT=./eval_outputs/soft_C
...
- **`--replan-steps 24` is mandatory for `C`** (`= num_action_per_block`, so 1 query = 1 block ≈ the next
  real observation). With the default `--replan-steps 5` the fed‑back frontier runs ~4.8× ahead of the
  robot and the conditioning is temporally wrong (the "imagined video runs to completion" symptom). See
  `LIBERO_EXPLICIT_CONDITIONING.md` §1.5.
...
- Imagined videos (if `--save_video_pred`) land in `./video_pred_output/checkpoint-$CKPT_STEP/`, one per
  completed episode (the last episode's buffer flushes on the next `reset()`).
...
MUJOCO_GL=egl MUJOCO_EGL_DEVICE_ID=0 python eval_utils/run_libero_eval.py --host 0.0.0.0 --port 8000 \
    ...
    --replan-steps 24 --video-out-path ./eval_outputs/smoke_C/videos \
    --metrics-out-path ./eval_outputs/smoke_C/metrics.json
```

> **Runtime.** `context_mode=C` re‑anchors on the current real observation and **rebuilds the KV cache
> every query** (heavier per query than baseline's incremental cache), but `--replan-steps 24` issues only
> **~1 query per block** (≈10 queries for a 220‑step `libero_spatial` episode vs ~44 at replan 5), so the
> two roughly offset. Use the baseline measurements as a rough guide: with **EGL** the bottleneck is
> policy **inference**, ~1.6× faster end‑to‑end than OSMesa. Ballpark ~0.5–1.5 min/episode for the shorter
> suites, ~2–3 min for `libero_10` (520 steps). Full 4 suites × 10 tasks × 5 trials = **200 episodes ≈
> ~3–4 h** (≈ **~2 h** at 3 trials/task). OSMesa (`MUJOCO_GL=osmesa`) is the robust CPU fallback if EGL
> won't initialize.
...
One `metrics.json` per suite under `eval_outputs/soft_C/<suite>/`. Top level keeps the original fields
(incl. `replan_steps: 24`) and adds `overall_mean_progress`; each `per_task[]` gains a `progress` block.
...
2. **`context_mode=C` requires `future_frame_shift`** (auto‑detected from the checkpoint config). Serve a
   future‑shift checkpoint, or force `--future_frame_shift True`. The episode‑start no‑op and the
   "aligned action in one forward" both depend on the shift (`LIBERO_EXPLICIT_CONDITIONING.md` §1.5).
3. **Always pair `C` with `--replan-steps 24`** (`= num_action_per_block`). At the default `replan 5` the
   fed‑back block runs ~4.8× ahead of the robot and the conditioning is temporally wrong. `baseline` is
   the only mode where a small `--replan-steps` is valid (it discards the video).
4. **`context_mode=baseline` ≠ feeding video back.** The default eval re‑grounds on the real frame and
   *discards* its prediction across queries; the cross‑query feedback only happens with
   `--context_mode C`. Drop `--context_mode C` / `--save_video_pred` (server) and `--replan-steps 24`
   (client) to reproduce the byte‑for‑byte baseline soft eval.
5. **CUDA toolkit / `nvcc` is mandatory** (§0). Without it the server dies at import with
...
6. **flash-attn:** no PyPI wheel for torch2.8/cp311 → install the prebuilt wheel by URL (§1a), or skip it
...
7. **`hf download` honors only the LAST `--include`** when the flag is repeated → use **one `--include`
   per command** (§2c). Silently downloads nothing for the dropped patterns otherwise.
8. **`--tokenizer_path` is mandatory on a fresh box.** The checkpoint's baked `tokenizer_path` is an
   absolute path from the training machine; pass `./checkpoints/umt5-xxl` to override it.
9. **Server = eager (default).** `torch.compile` measured ~0% here (inference‑bound) with warmup +
   recompile risk. Blackwell must stay eager anyway (`ptxas` can't target `sm_103a`).
10. **Rendering = EGL (default), ~1.6× faster** than OSMesa (`export MUJOCO_GL=egl MUJOCO_EGL_DEVICE_ID=0`).
    The doc's loop auto‑retries a suite if EGL SIGABRTs; if EGL won't init at all, fall back to
    `MUJOCO_GL=osmesa`.
11. **Progress thresholds** (`--approach-dist`, `--place-dist`) are heuristics for the *intermediate*
...
12. **Verify shard completeness — and recovering a missing *frozen* shard.** Some steps on the mirror can
    be incomplete (e.g. missing `model-00001/00002-of-00006` won't load). Confirm the local checkpoint has
    every `model-0000N-of-0000M.safetensors` referenced by `model.safetensors.index.json` before serving.
    **If the missing shard holds only frozen weights it is recoverable without a re‑upload:** shards 1–2
    are entirely `action_head.text_encoder` (the frozen umt5‑xxl T5 encoder) and are **byte‑identical
    across all checkpoints** (`sha256` of `model-00001/2` match between any two complete steps; only the
    trained DiT shards differ). So fill them by copying that shard from any complete checkpoint, e.g.
    `cp checkpoint-32000/model-0000{1,2}-of-00006.safetensors checkpoint-15000/`. (Trained shards —
    DiT/projector/action‑head — are step‑specific and **cannot** be reconstructed this way.) The
    shard→tensor map is identical across steps, so the index still matches.
13. **Disk:** model‑only checkpoint ≈ 30 GB; Wan backbone ≈ 28 GB; tokenizer a few MB; rollout MP4s a
    few hundred MB (plus the imagined `video_pred_output/` MP4s if `--save_video_pred`). Rollout videos are
    saved by default — add **`--no-save-videos`** if you only want `metrics.json`.
14. **`.env` is git‑ignored — never commit it.**
```
