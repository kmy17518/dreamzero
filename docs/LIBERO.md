...
Train from `/root/dreamzero` (on the `libero_al_only` branch); the launcher sets
`PYTHONPATH=$DREAMZERO_ROOT` so the distributed workers import the on-branch `groot`. **Run the
env-prefixed launcher as ONE line.** Pick a **unique** `WANDB_RUN_ID` / `OUTPUT_DIR` / `UPLOAD_REPO`
(the wandb *project* stays `dreamzero_libero`, so curves overlay with the §10/§11 runs).
...
- `NUM_GPUS=7 CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6` trains on GPUs 0–6 and leaves **GPU 7 for the eval
  watcher**. On a fully free node use `NUM_GPUS=8` and drop `CUDA_VISIBLE_DEVICES`.
- **bs=1** (same model limitation as §6.1); grow the global batch via
  `training_args.gradient_accumulation_steps=N` and/or more GPUs.
