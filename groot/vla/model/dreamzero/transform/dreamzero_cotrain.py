            ...
            for elem in features:
                ...
                try:
                    ...
                    if num_views > 1 and elem["embodiment_id"] == embodiment_tag_mapping[EmbodimentTag.AGIBOT.value]:
                        ...
                        processed_item = "A multi-view video shows that a robot " + processed_item.lower() + " The video is split into two views: the left view shows the agent (front) camera and the right view shows the camera view from the robot's wrist. The robot " + processed_item.lower()
                    else:
                    ...
                    if num_views > 1 and elem["embodiment_id"] == embodiment_tag_mapping[EmbodimentTag.AGIBOT.value]:
                        ...
                        item = "A multi-view video shows that a robot " + str(item).lower() + " The video is split into two views: the left view shows the agent (front) camera and the right view shows the camera view from the robot's wrist. The robot " + str(item).lower()
                    else:
                        raise ValueError(f"Embodiment ID {elem['embodiment_id']} not supported.")   
...
class DreamTransform(InvertibleModalityTransform):
    ...
    def _prepare_video(self, data: dict):
        ...
        if images.shape[0] > 1:
            ...
            # For LIBERO: 2 views placed side by side in a single row.
            # Layout: [agentview | wrist]  -> (h, 2w)
            # View indices (expected):
            # - View 0: agentview (front) camera
            # - View 1: wrist camera
            if self.embodiment_tag == EmbodimentTag.LIBERO_SIM and v >= 2:
                agentview = images[0]  # (t, c, h, w)
                wrist = images[1]      # (t, c, h, w)
                concat_images = np.zeros((1, t, c, h, 2 * w), dtype=images.dtype)
                concat_images[0, :, :, :, :w] = agentview
                concat_images[0, :, :, :, w:] = wrist
                return concat_images
