#!/bin/bash
# DreamZero LIBERO Training Script with Wan2.2-TI2V-5B backbone (FULL fine-tune, FROM SCRATCH, NO LoRA)
#
# Trains DreamZero on the LIBERO benchmark using the 5B Wan2.2 backbone. This is a full
# fine-tune (train_architecture=full, save_lora_only=false) initialized only from the
# Wan2.2-TI2V-5B base video weights -- it does NOT load any DreamZero checkpoint
# (no pretrained_model_path), i.e. the action/state heads are trained from scratch.
#
# Usage:
#   DATA_ROOT=/root/dreamzero/data/libero90_gear bash scripts/train/libero_training_wan22.sh
#
...
#
# Prerequisites:
#   - LIBERO dataset converted to GEAR format at DATA_ROOT
#       python scripts/data/convert_libero_to_gear.py --src <libero_lerobot> --out <DATA_ROOT> --embodiment-tag libero
...
if [ -n "$DREAMZERO_ROOT" ] && [ -d "$DREAMZERO_ROOT/groot" ]; then
    ...
    DREAMZERO_ROOT="${DREAMZERO_ROOT:-/root/dreamzero}"
fi
...
# ============ USER CONFIGURATION ============
DATA_ROOT=${DATA_ROOT:?"Set DATA_ROOT to your GEAR-converted LIBERO dataset (run convert_libero_to_gear.py)"}
OUTPUT_DIR=${OUTPUT_DIR:-"$DREAMZERO_ROOT/checkpoints/dreamzero_libero_wan22_full_finetune"}
...
# Validate dataset
if [ ! -d "$DATA_ROOT" ]; then
    echo "ERROR: LIBERO GEAR dataset not found at $DATA_ROOT"
    echo "Convert with: python scripts/data/convert_libero_to_gear.py --src <libero_lerobot> --out $DATA_ROOT --embodiment-tag libero"
    exit 1
fi
if [ ! -f "$DATA_ROOT/meta/embodiment.json" ]; then
    echo "ERROR: $DATA_ROOT/meta/embodiment.json missing -- run convert_libero_to_gear.py first"
    exit 1
fi
...
# Full fine-tune from scratch: train_architecture=full, save_lora_only=false, ZeRO-2 (+ optional CPU offload).
# NO pretrained_model_path -> heads trained from scratch (only Wan2.2 base video weights are loaded).
