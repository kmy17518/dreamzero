...
#
# FULLY (BIDIRECTIONALLY) DECOUPLED ACTION/DYNAMICS VARIANT: this launcher trains the SAME joint
# (dynamics + action) flow-matching objective as scripts/train/libero_training_wan22.sh, but the DiT
# attention is modified in BOTH directions:
#   - ACTION tokens are NOT allowed to attend to the noisy / being-generated ("future") VIDEO tokens
#     (decouple_action_dynamics, same as scripts/train/libero_training_wan22_decoupled.sh), AND
#   - the noisy / being-generated VIDEO ("dynamics") tokens are NOT allowed to attend to the ACTION
#     block (decouple_dynamics_action).
# Both pathways still attend to the clean observation context (current + past frames) and their own
# state block, so they share ONLY the real observations + state -- there is no cross-talk between the
# predicted action and the being-generated video in either direction. Neither loss is dropped.
#
# IMPORTANT modeling note: with decouple_dynamics_action on, the video model can no longer condition
# its next-frame prediction on the action, so it becomes an action-UNCONDITIONED video predictor. If
# you want the video to stay an action-conditioned world model, use the one-directional variant
# (scripts/train/libero_training_wan22_decoupled.sh) instead.
#
# The ONLY difference vs. scripts/train/libero_training_wan22.sh is the action_head config (selects
# wan_flow_matching_action_tf_wan22_decoupled_bidir, which sets BOTH
# diffusion_model_cfg.decouple_action_dynamics=true and decouple_dynamics_action=true) and a separate
# default OUTPUT_DIR so the runs never collide. The original joint-loss path, the action-loss-only
# path, and the one-directional decoupled path are all unaffected (the new behavior is gated behind
# config flags that default to off).
#
# Usage:
#   bash scripts/train/libero_training_wan22_decoupled_bidir.sh
#
...
# silently miss the code in THIS copy (e.g. the decouple_*_* attention) -> it would train the JOINT
# (coupled) attention instead. Prepend this repo so workers import `groot` from here.
...
# Separate default OUTPUT_DIR so the bidirectional-decoupled run never collides with the joint-loss,
# action-loss-only, or one-directional decoupled runs.
OUTPUT_DIR=${OUTPUT_DIR:-"$DREAMZERO_ROOT/checkpoints/dreamzero_libero_wan22_decoupled_bidir"}
...
"${RUN_CMD[@]}" \
    ...
    num_views=2 \
    model=dreamzero/vla \
    model/dreamzero/action_head=wan_flow_matching_action_tf_wan22_decoupled_bidir \
