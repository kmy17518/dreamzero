...
#
# ACTION-LOSS-ONLY VARIANT: this launcher trains on ONLY the action flow-matching loss
# (train/action_loss). The video dynamics loss (train/dynamics_loss) is still computed and
# logged for monitoring, but it is excluded from the optimized loss (no gradient). This is the
# only difference vs. scripts/train/libero_training_wan22.sh; the original joint-loss training
# path is unchanged. It also defaults to a separate OUTPUT_DIR so the two runs never collide.
#
# Usage:
#   bash scripts/train/libero_training_wan22_action_loss_only.sh
#
...
# silently miss the code in THIS copy (e.g. the action_loss_only gating) -> it would train the
# JOINT loss instead. Prepend this repo so workers import `groot` from here.
...
# Separate default OUTPUT_DIR so the action-loss-only run never collides with the joint-loss run.
OUTPUT_DIR=${OUTPUT_DIR:-"$DREAMZERO_ROOT/checkpoints/dreamzero_libero_wan22_action_loss_only"}
...
"${RUN_CMD[@]}" \
    ...
    num_views=2 \
    model=dreamzero/vla \
    model/dreamzero/action_head=wan_flow_matching_action_tf_wan22_action_loss_only \
